...
(function () {
  ...
  function countUp(el) {
    ...
    var accentHtml = accent ? accent.outerHTML : "";
    var obj = { v: 0 };
    window.gsap.to(obj, {
      v: target,
      duration: 1.8,
      ease: "power2.out",
      onUpdate: function () {
        el.innerHTML = prefix + Math.round(obj.v) + accentHtml;
      },
      onComplete: function () {
        el.innerHTML = prefix + target + accentHtml;
      }
    });
  }
  ...
  tl.to(".header", { opacity: 1, duration: 0.8 }, 0)
    .to(".hero-title .line > span", { y: 0, duration: 1.1, stagger: 0.09, ease: "power4.out" }, 0.15)
    ...
    .to(".links path", { strokeDashoffset: 0, duration: 1.4, stagger: 0.05, ease: "power2.inOut" }, 0.75)
  ...
  if (stage && inner && window.matchMedia("(pointer:fine)").matches) {
    ...
    stage.addEventListener("mouseleave", function () {
      gsap.to(inner, { rotationY: 0, rotationX: 0, duration: 1, ease: "elastic.out(1, 0.5)" });
    });
  }
  ...
  [".cases", ".stance", ".steps", ".plans", ".group-grid", ".coverage"].forEach(function (sel) {
    document.querySelectorAll(sel).forEach(function (row) {
      rise(row.children, { y: 60, duration: 0.9, stagger: 0.1, trigger: row, start: "top 82%" });
    });
  });
